// g++ -pthread -std=c++14 -fopenmp -o gabow.out gabow.cpp -D LOCAL_DEFINE
#include <algorithm>
#include <cassert>
#include <cstring>
#include <chrono>
...
void bellman_ford(int &src, int &vertices, int &num_of_threads, vector<vertex>* graph_poi) {
...
int main(int argc, char **argv) {
  ...
  */
  ...
  if (argc <= 3) {
    ::std::cerr << "Num of threads not specified" << ::std::endl;
    abort();
  }
  int num_of_threads = atoi(argv[3]);
  trace3(input_filename, output_filename, num_of_threads);
  ...
  int src = 0;
  thread t(bellman_ford, ref(src), ref(vertices), ref(num_of_threads), graph);
  t.join();
